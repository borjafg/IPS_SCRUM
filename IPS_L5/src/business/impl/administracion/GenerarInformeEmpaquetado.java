...
public class GenerarInformeEmpaquetado implements Command {
	...
	private List<DatosInformeEmpaquetado> informe;
	private List<Map<String, Object>> plantillaFechas;

	@Override
	public Object execute() throws BusinessException {
		try {
			inicializarInforme(OrdenTrabajoFinder.findOrdenTrabajoTerminadaEmpaquetada_MasAntigua(), new Date());
			...
			for (Almacenero al : almaceneros) {
				List<Object[]> info = OrdenTrabajoFinder.findNumOrdenTrabajoEmpaquetadasDia(al);

				plantillaCompleta = rellenarPlantilla(info);

				informe.add(new DatosInformeEmpaquetado(al, plantillaCompleta));
			}
	...
	private void inicializarInforme(Date fechaIni, Date fechaFin) throws BusinessException {
		...
		informe = new ArrayList<DatosInformeEmpaquetado>();
